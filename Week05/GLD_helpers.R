# based on https://mpra.ub.uni-muenchen.de/43333/3/MPRA_paper_43333.pdf

S_ <- function(p, chi, xi) {
  stopifnot(is.numeric(p), is.numeric(chi), is.numeric(xi),
            length(chi) == 1, length(xi) == 1)
  alpha <- 0.5 * (0.5 - xi) / sqrt(xi * (1 - xi))
  beta <- 0.5 * chi / sqrt((1 + chi) * (1 - chi))
  if (chi != 0 || xi != 0.5) {
    if (abs(alpha) != beta) {
      s <- alpha + beta
      d <- alpha - beta
      if (alpha == -Inf) x <- rep(0, length(p))
      x <- (p ^ s - 1) / s - ( (1 - p) ^ d - 1 ) / d
  ...
  } else x <- log(p) - log1p(-p) # chi == 0 & xi == 0.5
  zeros <- p == 0
  if (length(zeros)) 
    x[zeros] <- ifelse(xi < 0.5 * (1 + chi), -1 / (alpha + beta), -Inf)
  ones <- p == 1
  if (length(ones))
    x[ones]  <- ifelse(xi < 0.5 * (1 - chi),  1 / (alpha - beta),  Inf)
  invalid <- p < 0 | p > 1
  if (length(invalid))
    x[invalid] <- NA_real_
  return(x)
...
qgld <- function(p, median, IQR, asymmetry, steepness) {
  stopifnot(is.numeric(IQR), length(IQR) == 1, IQR >= 0,
            is.numeric(asymmetry), length(asymmetry) == 1, 
            asymmetry >= -1, asymmetry <= 1,
            is.numeric(steepness), length(steepness) == 1,
            steepness >= 0, steepness <= 1, is.numeric(p))
  CHI <- abs(asymmetry)
  if (CHI < 1) {
    z <- S_(c(lower = 0.25, median = 0.5, upper = 0.75), asymmetry, steepness)
    return (median + IQR * (S_(p, asymmetry, steepness) - z["median"]) / 
                           (z["upper"] - z["lower"]))
  } else {
    if (steepness != 0) stop("'steepness' must be zero when asymmetry = ", asymmetry)
...
GLD_solver <- function(lower_quartile, median, upper_quartile, 
  ...
  obj_xi <- function(xi) {   # chi will be local
    qgld(alpha, median, IQR = upper_quartile - lower_quartile, chi, xi) - other_quantile
  }
  ...
  if (check) {
    low <- qgld(0, median, IQR = upper_quartile - lower_quartile,
                asymmetry = a_s[1], steepness = a_s[2])
    if (alpha > 0 && low > -Inf) warning("solution implies a bounded lower tail at ", low)
    high <- qgld(1, median, IQR = upper_quartile - lower_quartile, 
                 asymmetry = a_s[1], steepness = a_s[2])
...
GLD_solver_LBFGS <- function(lower_quartile, median, upper_quartile,
  ...
  fn <- function(theta) {
    ...
    r <- c((qgld(alpha, median, IQR, theta[1], theta[2]) - other_quantile) / IQR, 
...
GLD_solver_bounded <- function(bounds, median, IQR, check = TRUE, ...) {
  obj <- function(theta) {
    ...
    out <- (bounds[1] - qgld(0, median, IQR, chi, xi)) ^ 2 +
           (bounds[2] - qgld(1, median, IQR, chi, xi)) ^ 2
    return(out)
  }
  ...
  if (check && sln$minimum > 1e-15) {
    low  <- qgld(0, median, IQR, asymmetry = a_s[1], steepness = a_s[2])
    high <- qgld(1, median, IQR, asymmetry = a_s[1], steepness = a_s[2])
